...
\ssnote{Maybe start with 'Following [CITES],'} We model the world as a volumetric function $\Phi(x) : \mathbf{R}^3 \to
...
\ssnote{It might be clearer if vectors and scalars were disambiguated. For example,
$\mathbf{x}$ and $u$. Also nice if unit vectors were capped like $\mathbf{\hat{r}}$ }
...
$\Phi$ by locally fitting a plane around $x$ \ssnote{using neighboring points?}, 
and using the distance to that
...
\begin{figure*}
  ...
  \label{fig:integration_diagram}
   ...
   surface from the camera, and a voxel that the ray passes through. The length of the ray from the point at
      ...
      hit probability of a ray $P\left(z = u \right) $, the pass
      probability $P\left(z< u\right)$, vs. $u$. The hit integration region is
      inside $[-\tau, \tau]$, wheras regions closer to the camera than  $\tau +
      \epsilon$ are in the space carving region (Section \ref{section:carving}). }
\end{figure*} 


This observation leads to the Truncated Signed Distance Function (TSDF)
\cite{Curless1996}. The TSDF is an approximation \ssnote{technically, this is not an approximation but 
a restriction to $\pm \tau$. Within the restriction, it is exact, no?} of the  Signed Distance
Function which only stores distances to the surface within a small truncation
distance, $\tau$ \ssnote{this is a very unclear way of stating that the TSDF stores everything between the $\pm \tau$ levelsets of $\Phi$
Isn't the TSDF defined as $\Phi(x),\forall x: |\Phi(x)|\leq\tau$?}.  All other parts of the field are undefined. The TSDF allows
us to maintain distance field data near the surface of objects, while ignoring
distance data far \ssnote{further} away from surfaces. We will refer to the TSDF as
$\Phi_{\tau}$.
...
probability distribution of the sensor \ssnote{not clear what this means}. To do this, we can introduce a global
...
\end{equation}
 ...
 \noindent and $K$ is a constant. \ssnote{This is very confusing. Why is this necessary?
 Also, $\alpha$ is a function of $\tau$, right? Otherwise, the above is not possible for 
 one $\alpha$. Maybe rewrite $\alpha$ as $\alpha_\tau$?}
 
Ideally, the weighting function should be determined by the probability distribution of the sensor 
\ssnote{again, not clear what this statement means}, and should represent
the probability that $\Phi(x - u r) = 0$ \ssnote{yes, this is what it should represent. Even if the sensor were perfect,
there would still be uncertainty about this value becuase we don't know the surface. Uncertainty in this value is due to both
sensor noise and uncertainty about the surface itself.}. 
It is possible \cite{Nguyen2012} to
directly compute the weight from the probability distribution of the sensor, and hence compute
the actual \emph{expected} signed distance function \ssnote{Not sure if this is true}. In favor of better
...
Here, we simply use a constant approximation $\alpha(u) = \frac{1}{2
...
\end{align}
where $\mathbf{c} \in \mathbf{R}^3$ is the color of the ray in RGB
space, $\alpha_c$ is a color weighting function, and $w_c = W_c(x -
ur)$. In both \cite{Bylow2013} and \cite{Whelan2013}, the color weighting
function is proportional to the dot product of the ray's direction and an estimate of the
...
\ssnote{RGB seems like a pretty weird color space to be averaging over. Did you try LAB, 
a more reasonable color space to do such operations?}
...
(nightime) outdoor scenes in real-time (Figure \ref{fig:overhead_night}).
