...
\begin{figure*} 
  ...
  \begin{subfigure}{0.45\linewidth}\centering
	 \includegraphics[width=0.5\textwidth]{img/aerial.jpg}
	 \caption{}
	 \label{fig:robot}
  \end{subfigure}
 	 \begin{subfigure}{0.45\linewidth}\centering
	...
	\begin{subfigure}{0.45\linewidth}\centering
	 ...
	 \label{fig:path_plan}
	 \end{subfigure}
      \caption{ \figref{fig:robot} shows an aerial robot with
      a \textit{Yellowstone} tablet device attached to the top. The
      device is light enough to allow the robot to fly.
      \figref{fig:path_plan} shows two motion plans for a
      simple 3D sphere robot, varying $\epsilon$, the minimum
      allowed robot radius. \figref{fig:drift} shows a top down view of a $\sim
      ...
      optimized offline using bundle adjustment, overlaid on the floorplan of the building.}
...
\ssnote{THIS NEEDS TO BE EDITED AND ADDED TO THE ACTUAL TEXT.}
  \ssnote{ We compare our spatial hashing (SH)
...
paused. After mapping is complete, the user can save the map to disk.
 ...
 robot on which we mounted a tablet device. 
