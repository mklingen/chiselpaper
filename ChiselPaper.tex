...
\begin{abstract}
We present a method for real-time house-scale (300 square meter or more)
3D reconstruction \cite{Hartley2004} using a mobile Google \textit{Tango} \cite{Tango}
device through the use of a dynamic spatially hashed \cite{SpatialHashing} truncated
signed distance field \cite{Curless1996} for mapping, and visual-inertial
odometry \cite{VINS} for localization. Our key insight is that by
aggressively culling parts of the scene that do not contain surfaces, we avoid needless
computation and wasted memory. We are able to reconstruct and render very large
(300 square meter) scenes at a resolution of 2-3cm in real time on a mobile
device without the use of GPU computing, with a memory footprint that is
approximately 84 percent smaller than \textit{Kinect Fusion} \cite{Newcombe}. We provide both
qualitative and  quantitative results on publicly available RGB-D datasets
\cite{FREIBURG}, and on datasets collected in real-time from two devices.
...
fields at each timestep, we avoid needless computation
(\tabref{table:meshingtimes}}) and memory allocation (\figref{fig:memory_data}) 
in areas far away from the sensor.
