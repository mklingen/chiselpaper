...
\begin{figure*}
  ...
  \label{fig:integration_diagram}
   ...
   surface from the camera, and a voxel that the ray passes through. The length of the ray from the point at
      ...
      approximation of the signed distance field $\Phi$, when $u$ is small.
      ...
      whereas regions closer to the camera than  $\tau + \epsilon$ are in the
      space carving region (\sref{section:carving}). }
\end{figure*} 
% 
...
\ref{fig:integration_diagram}), we \chisel away data that has a nonpositive
stored SDF. Algorithm \ref{alg:TSDF}, line \ref{alg:line:voxel_carve} shows how this is
accomplished. 
