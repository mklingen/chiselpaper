...
\begin{algorithm} 
	...
	\begin{algorithmic}
		...
		\For {$x \in \mathbf{R^3}$}
			\State {$\Phi_{\tau}(x) \gets \text{undefined} $}
			\State {$W(x) \gets 0$}
		\EndFor
		...
		\For {$t \in [1 \ldots T]$} 
			...
			\For {$\{o, x\} \in Z_t$} 
			    ...
			    \For{$u \in [\tau + \epsilon, z]$}
			    	\If {$\Phi_{\tau}(x - ur) \leq 0$ }
				    	\State {$\Phi_{\tau}(x - ur) \gets \text{undefined}$}
						\State {$W(x - ur) \gets 0$}
					\EndIf
			    \EndFor
				\LineComment{Within the hit region:}a
				\For{$u \in [-\tau, \tau]$} 			 
				 \LineComment{Update the TSDF and weight.}
				   \State{$w_u = W(x - ur)$} 				  
				   \State{$\alpha_u = \alpha(u)$}
				   \State{$\Phi_{\text{old}} \gets  \Phi_{\tau}(x - ur) $}
					\State {$\Phi_{\tau}(x - ur) \gets \frac{w_u \Phi_{\text{old}} + \alpha_u
					u}{w_u +\alpha_u}$}
					\State {$W(x - ur) \gets w_u+ \alpha _u$}
				\EndFor
			\EndFor
		\EndFor
	\end{algorithmic}
