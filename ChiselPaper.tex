...
\begin{figure*}
  \centering
	 ...
	 \end{subfigure}
      \caption{Figure \ref{fig:map_device} \ssnote{please use the figref macro for consistency} shows our algorithm recording a map
...
mobile devices such as phones and tablets. \ssnote{It'd be nice to add a sentence here 
stating why performing 3D reconstruction with these devices is intreresting or useful}	
Because these devices have limited
computing power, memory, and graphics capabilities, performing real-time 3D
reconstruction on them remains challenging. Existing approaches are either too
inefficient for use on mobile devices \ssnote{this is a very negative statement, 
rehprase with maybe 'existing devices often rely on GPUs which are unavailable on these devices'}
, or only consider sparse reconstructions \ssnote{why? and are these sparse reconstructions implemented on mobile devices?}.
\ssnote{What's the state of the art on mobile devices?}
We propose an efficient method for real-time dense 3D reconstruction using
chunked, dynamic spatial hash maps \cite{SpatialHashing} of truncated signed
distance volumes \cite{Curless1996} \ssnote{this is a bunch of jargon. I'd put this below the key insight and explain how it connects to the insight}. 
\ssnote{State your key insight. 'Our key insight is ...'}
\ssnote{Hmm, it's not clear at all from this abstract that the key reason why we're able to simplify the 
process is because we have pose from hardware.}
We show that our method is able to
reconstruct and render very large scenes \ssnote{say how big, 'of the order of 50 square meters' or something like that}
in real-time using only the phone's
CPU and the fixed function graphics pipeline, with a minimal memory footprint
in comparison to existing approaches. 
\ssnote{This is too bold a claim. What are the tradeoffs?}
We provide both qualitative and
...
full-sized machines \ssnote{should we call these PCs?}, mobile devices have severely \ssnote{much, don't be so dramatic} 
limited processing power,
memory requirements \ssnote{what does it mean to have limited memory requirement}, and graphics \ssnote{not clear what this means, graphical rendering? dedicated graphics processing?} capabilities \ssnote{need to back this up with numbers}. Further, the depth sensors
...
and have much more undesirable nonlinear distortion and noise \ssnote{need to back this up with a citation and/or example}.
\ssnote{Because I have no idea how real-time 3D reconstruction is done, I have no appreciation for 
why limited processing and bad sensors are big challenges to overcome.}
...
\ssnote{I learned nothing from the above paragraph. I still don't know how any of this
is done.}
...
with a low-powered mobile device with severe limitations on memory usage \ssnote{replace with 'limited memory'},
processing power, and graphics capabilities, relatively high-quality,
large-scale real-time 3D reconstruction is feasible \ssnote{Again, this is just a repetition of what I've read until now}. 
...
\ssnote{I have no idea what this means and why this is significant. Also, is this our only contribution: using a dynamic spatial hash map? 
Seems pretty lame. And no mention yet about why we are able to exploit pose information.}

\ssnote{Ends abruptly. The introduction must be a self-contained unit that expands on the abstract, which 
is a self-contained unit that expands on the title. What are our results? How well do we do? What are our limitations?}

\ssnote{Also, \figref{fig:first_figure} not mentioned in the text! It needs to be the centerpiece of what we talk about.}
...
vision \ssnote{unnecessary sentence}. Mapping paradigms generally fall into one of two categories:
...
quality 3D reconstruction \ssnote{I don't understand this statement. what components aren't?}.
...
procedure. While simple, point clouds fail to capture local scene structure \ssnote{why not? don't get it}, are highly 
...
\begin{algorithm} 
	...
	\label{alg:TSDF}
	\begin{algorithmic}[1]
		\LineComment{Initialize TSDF}
		\For {$\mathbf{x} \in \mathbf{R^3}$}
			\State {$\Phi_{\tau}(\mathbf{x}) \gets \text{undefined} $}
			\State {$W(\mathbf{x}) \gets 0$}
		\EndFor
		...
		\For {$t \in [1 \ldots T]$} 
			...
			\For {$\{\mathbf{o}_t, \mathbf{x}_t\} \in Z_t$} 
				...
				\For{$u \in [-\tau, \tau]$} 			 
					\State {$\mathbf{v}_c \gets \mathbf{x} - u \mathbf{r}$}
				    \State{$w_{\text{old}} \gets W(\mathbf{v}_c)$} 				  
				    \State{$\alpha_u \gets\alpha(u)$}
				    \State{$\Phi_{\text{old}} \gets  \Phi_{\tau}(\mathbf{v}_c) $}
				   \LineComment{Update the TSDF and weight.}
					\State {$\Phi_{\tau}(\mathbf{v}_c) \gets \frac{w_{\text{old}} 
					\Phi_{\text{old}} + \alpha_u u}{w_{\text{old}}  +\alpha_u}$}
					\label{alg:line:tsdf_update}
					\State {$W(\mathbf{v}_c) \gets w_{\text{old}} + \alpha _u$}
				\EndFor
			\EndFor
		\EndFor
	\end{algorithmic}
...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
		...
		\State{$z_p \gets \mathbf{Depth}(\mathbf{Project}(\mathbf{v}_c))$}
		\State{$u \gets z_p - z$} 
		\LineComment {Dynamic truncation distance.}
