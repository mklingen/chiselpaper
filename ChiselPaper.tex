...
Mapping}, which divides the world into a 2D grid of axis-aligned cells (i.e
Voxels), each of which contains an occupancy probability. Occupancy grids
provide a principled alternative to simple point clouds. They preserve local
...
Curless \cite{Curless1996} introduced an alternative to occupancy grids which,
instead of storing the occupancy probability for each voxel, stores an
approximation of the signed distance field of the scene. The signed distance
...
space where the value of the SDF is zer. The structure introduced in
