...
may be on the order of several gigabytes  (\sref{section:memory}), and the
resulting reconstructed mesh has on the order of 1 million vertices.  Previous
works \cite{Whelan2013, WhelanLoopClose} have extended the \TSDF to larger
scenes by storing a \textit{moving} voxelization and throwing away data
incrementally -- but we are interested in preserving the volumetric data for
later use.
...
and scan-to-model ICP, since the depth data is hardware-limited to only 3-6Hz,
which is too slow to directly estimate the pose. Instead, we incrementally
adjust the VIO pose estimate by registering each new depth scan with to model
built up over time.

Given a pose estimate of the sensor at time $t$, $H_{t}$, we define a cost
...
\noindent which can be minimized by setting its partial derivative with respect
to the pose to zero:

\begin{align}
     \frac{\partial c}{\partial H_t} &= \sum_{\mathbf{x}_i \in Z_{t}}
     \frac{\partial}{\partial H_t} \Phi_{\tau}(H_{t} \mathbf{x}_i)^2 \\
        &= \frac{1}{2}\sum_{\mathbf{x}_i \in Z_{t}} \Phi_{\tau}(H_{t}
        \mathbf{x}_i) \mathbf{J}_i^T \nabla\Phi_{\tau}(H_{t} \mathbf{x}_i)
\end{align}

\noindent where $\mathbf{J}_i$ is the Jacobian $\frac{\partial
\mathbf{x_i}}{\partial H_t}$, and $\nabla \Phi_{\tau}$ is the gradient of the
\TSDF computed via central finite differencing.  We can minimize the error of
each scan through gradient descent, applying a correction of $H_t^{(k + 1)} = H_t^{(k)}
-\lambda\frac{\partial c}{\partial H_t}$ with $\lambda$ as a convergence rate,
until convergence. To prevent the system from drifting too far from the VIO
trajectory, we reset the pose offset each time we begin gradient descent.
