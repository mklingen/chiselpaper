...
\begin{abstract}
Google's \textit{Project Tango}\cite{Tango} has made active depth sensing
available to mobile devices such as phones and tablets. Because of limited computing power,
memory, and graphics capabilities, dense 3D reconstruction on mobile devices
remains challenging. Existing approaches are either too inefficient for use on
mobile devices, or only consider sparse reconstructions. We propose an efficient
method of real-time dense 3D reconstruction using a dynamic spatial hash map
\cite{SpatialHashing} of truncated signed distance field volumes
\cite{Curless1996}. We show that our method is able to reconstruct and render
very large scenes in real time using only the phone's CPU and the fixed
function graphics pipeline, with a minimal memory footprint in comparison to
existing approaches. We provide both qualitative and quantitative results on
publicly available RGB-D datasets \cite{FREIBURG}, and on datasets collected in
real-time from two kinds of devices.
\end{abstract}
...
sensors are now commonplace in average market phones. Recently, mobile phone
manufacturers have integrated active depth sensors into mobile devices.
Depth sensors open up interesting possibilities for 3D reconstruction, mapping,
...
\cite{Tango} phones and tablets use a small active infrared projection depth
sensor combined with high-performance IMUs and a wide field of view camera. 
Mobile devices with depth sensors can also provide a light-weight integrated
sensing platform for small mobile robots such as quadcopters.

We are interested in the problem of dense real-time 3D reconstruction on a
handheld mobile device. The task is to extract the true 3D geometry and color of
a real scene from a sequence of noisy sensor readings.  Solutions to this problem are
useful for mobile robot navigation, indoor localization, mapping, and object
scanning. The problem is ill-posed, since it requires localization as well as mapping (and hence is a variant of
the SLAM problem.) In this work, we consider only the mapping portion of the
problem. Given a series of ground truth poses of the mobile sensor, how do we
construct a map in real time? This task is challengning due to the hardware
limitations of the device. Compared to full-sized machines, mobile devices have
severely limited processing power, memory, and graphics capabilities. Further,
the depth sensors available on these devices are much more limited than their
full-sized counterparts; they are typically lower resolution, have slower 
refresh rates, and have much more undesirable nonlinear distortion and noise
(Sec. \ref{section:hardware}).
...
computation \cite{TanskanenMetric} for dense 3D mapping. In this work, we
propose a novel algorithm for efficient real-time 3D reconstruction on mobile devices
with onboard depth sensors. Our algorithm relies on using a dynamic spatial hash map
\cite{SpatialHashing} to directly store distance field data, which greatly
reduces the amount of memory required to store the reconstruction as compared
to state of the art approaches. We show that even with a low-powered mobile
device with severe limitations on memory usage, processing power, and graphics
capabilities, relatively high-quality, large-scale real-time 3D reconstruction
is feasible. 
...
tree requires $\mathcal{O}(K)$ heap pointer dereferences in the worst case.
